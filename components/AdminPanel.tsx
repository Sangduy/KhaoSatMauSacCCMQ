...
export const AdminPanel: React.FC = () => {
  ...
  const [isBackingUp, setIsBackingUp] = useState(false); // ĐÃ GIỮ LẠI ĐỂ KHÔNG LỖI RENDER
  ...
  useEffect(() => {
    ...
    if (selectedRecord) {
      ...
      if (savedDraft) {
        ...
        setEditingClinicalData(JSON.parse(JSON.stringify(selectedRecord.clinicalData || {}))); // Load gốc an toàn
      }
    } else {
  ...
  const handleBackupToDrive = async () => {
    const url = getGoogleScriptUrl();
    if (!url || records.length === 0) return alert("Kiểm tra URL hoặc dữ liệu trống!");
                                ...
                                {selectedRecord.asScores && Object.entries(selectedRecord.asScores).map(([key, val]) => {
                                   ...
                                   return (
                                     <div key={key} className="flex justify-between items-center p-2 rounded bg-gray-50">
                                       ...
                                       <span className={`font-bold ${key !== 'binhHoa' && (val as number) >= 30 ? 'text-red-600' : 'text-gray-800'}`}>
                                         {val}
                                       </span>
                                     </div>
                                   );
